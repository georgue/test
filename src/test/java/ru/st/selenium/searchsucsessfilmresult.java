...
public class searchsucsessfilmresult extends logintest {
		...
		public void testUntitled(){
		   ...
		   try{
			...
			wait_elem_search_again.sendKeys("Большой куш", Keys.ENTER);
			
			
			//get a new film list
			// подумать как получить список фильмов если есть другие фильмы < //div[@class="title"] >
			
			WebElement new_listfilm = wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[contains(text(),\"Большой куш\")]")));
			//String Str_new_listfilm = new_listfilm.getText();
			
			assertNotEquals(wait_load, new_listfilm, "поиск сломался");
